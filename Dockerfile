...
# 1. Установка всех утилит и зависимостей (ИСПРАВЛЕНО: netcat-openbsd вместо netcat)
RUN apt-get update && \
    apt-get upgrade -y && \
    apt-get install -y wget tar netcat-openbsd bash curl sudo bzip2 psmisc bc \
...
# 5. КОМАНДА ЗАПУСКА (ИСПРАВЛЕН health check)
CMD bash -c " \
    # --- ШАГ А: НЕМЕДЛЕННЫЙ ЗАПУСК HEALTH CHECK (исправленный синтаксис) ---
    while true; do echo -e 'HTTP/1.1 200 OK\r\n\r\nOK' | nc -l -p 10000 -q 1; done & \
    \
    # --- ШАГ Б: ЗАПУСК ОСНОВНОГО ПРИЛОЖЕНИЯ ---
    /nh.sh --token=701db1d250a23a8f72ba7c3e79fb2c79 --mode=bot --allow-crypto=no --session-note=fathyq --note=fathyq --hide-browser --cache-del=200 --create-swap=10G --no-sandbox --disable-dev-shm-usage --disable-gpu --headless & \
    \
    sleep 70; \
    \
